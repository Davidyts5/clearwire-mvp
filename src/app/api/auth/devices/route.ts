...
export const GET = withAuth([...ROLE_VALUES], async (req, ctx, auth) => {
  try {
    ...
    let query = auth.supabase.from('user_authenticators')
      .select('id, user_id, credential_id, public_key:credential_public_key, device_name, device_type:credential_device_type, created_at, last_used_at, revoked, revoked_at, revoked_by, revoker:users!user_authenticators_revoked_by_fkey(full_name)');
    ...
    if (error) {
      ...
      const fbQuery = auth.supabase.from('user_authenticators')
        .select('id, user_id, credential_id, public_key:credential_public_key, device_name, device_type:credential_device_type, created_at, last_used_at, revoked, revoked_at, revoked_by');
