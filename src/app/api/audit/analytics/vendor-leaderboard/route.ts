...
export const GET = withAuth([ROLES.AUDITOR, ROLES.CFO], async (req, { params }, auth) => {
  try {
    ...
    wires.forEach(wire => {
      // Use vendor_name_snapshot if vendor_id is null for some legacy rows
      ...
      if (wire.risk_reasons) {
        try {
          const parsed = JSON.parse(wire.risk_reasons);
          parsed.forEach((r: any) => {
            if (typeof r === 'object' && r !== null) {
              if (r.code === 'BANK_CHANGED' || r.code === 'SWIFT_CHANGED') {
                isFlagged = true;
              }
    ...
    result.sort((a, b) => b.flag_count - a.flag_count || b.avg_risk_score - a.avg_risk_score);
