  ...
  useEffect(() => {
    const fetchWiresAndContext = async () => {
      try {
        ...
        if (session) {
          const { data: userData } = await supabase.from('users').select('approval_limit').eq('id', session.user.id).single();
    ...
    <div className="max-w-5xl mx-auto mt-10 px-4 sm:px-6 lg:px-8">
      <div className="bg-slate-900 text-white p-6 rounded-t-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-lg">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight">Controller Approval Portal</h1>
                 ...
                 return (
                  <tr key={req.id} className={`hover:bg-slate-50 transition-colors ${canApprove ? 'bg-blue-50/30' : ''}`}>
                    <td className="px-6 py-4 font-mono text-xs">
                      {req.amount > limit ? (
                        <span className="text-slate-400">Requires CFO</span>
                      ) : (
                        <Link href={`/approve/${req.id}`} className={`flex items-center gap-1 font-semibold underline ${canApprove ? 'text-blue-600 hover:text-blue-800' : 'text-slate-500'}`}>
