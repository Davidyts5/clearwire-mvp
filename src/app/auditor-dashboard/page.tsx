  ...
  const handleGenerateReport = async (e: any) => {
    ...
    try {
      const res = await fetch('/api/audit/reports/generate', {
        ...
        body: JSON.stringify({ startDate: reportStart ? new Date(reportStart).toISOString() : new Date().toISOString(), endDate: reportEnd ? new Date(reportEnd).toISOString() : new Date().toISOString() })
      });
      if (!res.ok) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95">
            <div className="p-6 border-b border-slate-100 flex items-center justify-between">
              <h2 className="text-xl font-bold flex items-center gap-2">Generate Compliance Report</h2>
            ...
            <form onSubmit={handleGenerateReport} className="p-6 space-y-4">
              <div>
                ...
                <input type="date" required value={reportStart} onChange={e => setReportStart(e.target.value)} className="w-full border p-2 rounded outline-none focus:border-blue-500" />
              </div>
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-1">End Date</label>
                <input type="date" required value={reportEnd} onChange={e => setReportEnd(e.target.value)} className="w-full border p-2 rounded outline-none focus:border-blue-500" />
              </div>
              <div className="flex justify-end gap-3 pt-4 border-t border-slate-100">
                <button type="button" onClick={() => setShowReportModal(false)} className="px-4 py-2 font-medium text-slate-600 hover:text-slate-900 text-sm">Cancel</button>
                <button type="submit" disabled={isGeneratingReport} className="bg-slate-900 hover:bg-slate-800 disabled:opacity-50 text-white px-6 py-2 rounded-lg font-bold text-sm shadow-sm flex items-center gap-2 transition-colors">
                  {isGeneratingReport ? <Loader2 size={16} className="animate-spin" /> : "Download PDF"}
                </button>
              </div>
            </form>
          </div>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => setShowReportModal(true)} className="bg-slate-900 hover:bg-slate-800 text-white px-4 py-2 rounded-lg font-bold text-sm shadow-sm transition-colors flex items-center gap-2">
            <FileDigit size={16}/> Generate Report
          </button>
          <a href="/api/audit/export-csv" className="bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 px-4 py-2 rounded-lg font-bold text-sm flex items-center gap-2 shadow-sm transition-colors">
